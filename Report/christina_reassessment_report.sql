...
set
  @prev_month = (
    SELECT
      DATE_FORMAT(
        DATE_SUB(
          STR_TO_DATE(CONCAT(@cur_month, '01'), '%Y%m%d'),
          INTERVAL 1 MONTH
        ),
        '%Y%m'
      )
...
WITH
  ...
  commission_data AS (
    SELECT
      ...
      MAX(
        IF(
          month = DATE_FORMAT(
            DATE_SUB(DATE(CONCAT(@month, '01')), INTERVAL 3 MONTH),
            '%Y%m'
          ),
          commission,
          0
      ...
      MAX(
        IF(
          month = DATE_FORMAT(
            DATE_SUB(DATE(CONCAT(@month, '01')), INTERVAL 2 MONTH),
            '%Y%m'
          ),
          commission,
          0
      ...
      (
        SELECT
          ...
          CASE
            ...
            AND DATE(paid_date) <= @last_day THEN 'settled'
        ...
        FROM
          loans l
        WHERE
          DATE(l.disbursal_date) <= @last_day
          AND l.cust_id IN (
            SELECT
              cust_id
            FROM
  ...
  loan_txns AS (
    SELECT
      ...
      COUNT(
        CASE
          WHEN DATEDIFF(lt.txn_date, l.due_date) > 1 THEN 1
        END
  ...
  par_loans AS (
    ...
    WHERE
      ...
      AND (
        l.paid_date IS NULL
        OR DATEDIFF(l.paid_date, l.due_date) > 5
      )
    GROUP BY
      r.cust_id
  ),
  ...
  loan_upgrade_check AS (
    SELECT
      cur.cust_id,
      cur.max_loan_principal > prev.max_loan_principal is_utilized
    FROM
      latest_loan_per_month prev
...
SELECT
  cwr.cust_id `Customer ID`,
  cwr.customer_name `Customer Name`,
  cwr.reg_date `Registration Date`,
  cwr.rm_name `RM Name`,
  COALESCE(tl.total_loan, 0) AS `Total Loan Count`,
  IF(
    al.assessment_limit = 0,
    'Ineligible',
    al.assessment_limit
  ) AS `Reassessed Limit`,
  lr.loan_principal `Last Loan Amount`,
  lr.last_loan_status `Last Loan Status`,
  CASE
  ...
  ua.current_limit `Upgraded Amount`,
  ua.upgraded_date `Upgraded Date`,
  IFNULL(luc.is_utilized, 0) `Utilized Upgrade`,
