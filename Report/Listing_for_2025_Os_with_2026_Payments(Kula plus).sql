...
select @country_code, @month, @last_day, @closure_date;
    ...
    payment AS (
        SELECT
            ...
            MAX(a.txn_date) as last_paid_date
    ...
    tt as (select
        ...
        CASE
            ...
            ELSE 'Ongoing'
            END as `Loan Status`,
        ifnull(fp.paid_principal,0) as `2026 Paid Principal`,
        ifnull(ffp.paid_fee,0) as `2026 Paid Fee`
    from
        os o 
        ...
        left Join futur_payments fp on fp.loan_doc_id = o.loan_doc_id
        left Join futrue_fee_payments ffp on ffp.loan_doc_id = o.loan_doc_id
