WITH
  reassessment AS (
    ...
    WHERE type = 'batch_reassessment'
      AND country_code = 'RWA' and date(created_at) = '2025-12-12'
  ),
  current_limit AS (
    SELECT
      a.cust_id,
...
SELECT
  ...
  COALESCE(cl.cur_limit, 0) `Current Limit`,
  COALESCE(LEAST(cl.cur_limit, b.last_upgraded_amount), 0) AS `Repayment Based Limit`,
  COALESCE(ll.last_loan_amount, 0) AS `Last Loan Amount`,
  ll.last_disbursal_date AS `Last Loan Date`,
  CASE WHEN ll.last_disbursal_date IS NULL OR DATEDIFF(curdate(), ll.last_disbursal_date) > 30 THEN "Inactive" ELSE "Active" END AS `Activity Status`,
  COALESCE(
    IF(
      ls.loans_needed_for_upgrade = 0,
      5,
      ls.loans_needed_for_upgrade
    ),
    5
  ) AS `Repayment Based Eligibility\n(Ontime payments required)`,
