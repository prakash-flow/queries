WITH
    toDateTime('2026-02-28 23:59:59') AS last_day,
    toDateTime('2026-02-28 23:59:59') AS payment_day,
    ...
    loan_base AS (
        SELECT
            ...
            l.loan_principal,
            l.flow_rel_mgr_id,
            CASE WHEN l.loan_purpose = 'adj_float_advance' THEN 'Kula'
            WHEN l.loan_purpose = 'float_advance' THEN 'Float Advance'
            ELSE l.loan_purpose END loan_purpose,
            l.due_date,
            dateDiff('day', l.due_date, last_day) as overdue_days,
            l.flow_fee
        FROM loans l
        INNER JOIN loan_txns lt ON lt.loan_doc_id = l.loan_doc_id
        WHERE lt.txn_type = 'disbursal'
            AND lt.realization_date <= toDateTime('2026-03-05 23:59:59')
    ...
    payments AS (
        ...
        FROM loan_txns
        WHERE realization_date <= toDateTime('2026-03-05 23:59:59')
          AND txn_date <= payment_day
    ...
    latest_visits AS (
        SELECT 
            ...
            visitor_name, 
            visit_end_time,
            dateDiff('day', visit_end_time, payment_day) since_last_visit,
            remarks
        FROM (
            SELECT 
                loan_doc_id, 
    ...
    collections_after_visit AS (
        ...
        WHERE lt.txn_type = 'payment'
          ...
          AND lt.realization_date <= payment_day
          AND lt.txn_date <= payment_day
...
SELECT
    ...
    toDate(p.last_payment_amount) as `Last Paid Amount`,
...
WHERE (`Principal OS` > 0 or `Fee OS` > 0) AND lb.overdue_days BETWEEN 6 AND 30;