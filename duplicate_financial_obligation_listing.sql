...
with disbursals as (
  ...
  from 
    ...
    left join financial_obligations_write_off lw on lw.stmt_txn_id = l.stmt_txn_id and lw.country_code = l.country_code and extract(year_month from lw.write_off_date) = @month
...
ifnull(reversal, 0) `Reversal Against Duplicate`,
ifnull(excess, 0) `Penalty Collected in Reversal`,
(Duplicate - ifnull(reversal,0)) `OS amount`
