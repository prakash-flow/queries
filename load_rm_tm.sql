...
customers AS (
    SELECT
        ...
        SUM(CASE WHEN COALESCE(rr.status, b.status) = 'disabled' THEN 1 ELSE 0 END) AS Disabled_Customers,
        SUM(CASE WHEN COALESCE(rr.status, b.status) = 'enabled' THEN 1 ELSE 0 END) AS Enabled_Customers,
        SUM(CASE WHEN DATEDIFF(@last_day, l.last_txn_date) <= 30 THEN 1 ELSE 0 END) AS Active_Customers,
        COUNT(*) AS Total_Customers
    FROM borrowers b
    LEFT JOIN recentReassignments r ON r.cust_id = b.cust_id
    LEFT JOIN latest_txn l ON l.cust_id = b.cust_id
...
loan_os AS (
    SELECT
        ...
        SUM(
            IF(
                GREATEST(lp.loan_principal - COALESCE(p.total_paid_principal,0),0) > 0 
                AND DATEDIFF(@last_day, lp.due_date) > 1,
                GREATEST(lp.loan_principal - COALESCE(p.total_paid_principal,0),0),
                0
            )
        ) AS Overdue_Amount,
        SUM(
...
SELECT 
    ...
    c.Enabled_Customers AS `Enabled Customers`,
    c.Disabled_Customers AS `Disabled Customers`,
    c.Active_Customers AS `Active Customers`,
    c.Total_Customers AS `Total Customers`,
