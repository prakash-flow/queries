...
SELECT
  ...
  a.acc_number,
  a.acc_prvdr_code,
  cb.assigned_band_amount AS band_amount
FROM customer_best_band cb
INNER JOIN accounts a ON a.cust_id = cb.cust_id
WHERE a.is_removed = 0 and has(acc_purpose, 'float_advance')
ORDER BY cb.assigned_band_amount DESC, cb.cust_id, a.acc_prvdr_code