SELECT 
    ...
    COUNT(DISTINCT IF(TIMESTAMPDIFF(YEAR, p.dob, '2024-12-31') <= 35, b.cust_id, NULL)) AS youth_count
FROM loans l
...
WHERE DATEDIFF('2024-12-31', txn_date) <= 30 
  AND DATE(txn_date) <= '2024-12-31' 
  AND EXTRACT(YEAR_MONTH FROM reg_date) <= 202412
  ...
  AND b.cust_id NOT IN (
    ...
    JOIN (
      ...
      FROM record_audits 
      WHERE DATE(created_at) <= '2024-12-31' 
