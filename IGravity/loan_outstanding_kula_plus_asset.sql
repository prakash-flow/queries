...
SELECT
    @month as Month,
  count(distinct l.loan_doc_id) as `Loans Outstanding`,
   la.loan_purpose as `Loan Purpose`,
    -- l.loan_doc_id,
    SUM(loan_os) AS principal_os,
    
    SUM(IF(par_days <=0,  loan_os, 0)) AS performing,
    SUM(IF(par_days between 1 and 30,  loan_os, 0)) AS overdue,
    SUM(IF(par_days between 31 and 60,  loan_os, 0)) AS par_30,
    SUM(IF(par_days between 61 and 90,  loan_os, 0)) AS par_60,
    SUM(IF(par_days between 91 and 120,  loan_os, 0)) AS par_90,
    SUM(IF(par_days between 121 and 180,  loan_os, 0)) AS par_120,
