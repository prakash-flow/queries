...
select sum(par_count_120),sum(par_loan_principal),sum(par_30),sum(par_60),sum(par_120),gender
from (
select 
	pri.loan_doc_id,
  sum(
  ...
  sum(
    IF(
      ...
      partial_pay
        ...
        SUM(IF(DATEDIFF(@last_day, due_date)  > 120,   if(principal - ifnull(partial_pay,0) > 0 , principal - ifnull(partial_pay,0), 0), 0)) AS par_120,pri.gender
from 
  (
    select 
      lt.loan_doc_id, 
#       loan_principal principal,
    	sum(amount) as principal,due_date, p.gender
    from 
      loans l 
      JOIN loan_txns lt ON lt.loan_doc_id = l.loan_doc_id 
    	join borrowers b ON b.cust_id=l.cust_id
    	join persons p  ON p.id=l.flow_rel_mgr_id
    where 
    ...
    group by lt.loan_doc_id,gender
#     ,loan_principal
  ) pri 
  left join (
    select 
  ...
  group  by pri.loan_doc_id,pri.gender having par_loan_principal >0 ) as a group by gender
  ;
